...
HSTDP - WARNING - this is a test warning.

>>> log.error("this is a test error.")
HSTDP - ERROR - this is a test error.

>>> log.error("this is another test error.")
HSTDP - ERROR - this is another test error.

>>> log.info("this is just informative.")
HSTDP - INFO - this is just informative.
...
>>> log.verbose("this is a test verbose message.")
HSTDP - DEBUG - this is a test verbose message.
...
HSTDP - DEBUG - this is a test verbose 60 message.
...
HSTDP - ERROR - Something bad happened and we trapped it : some value was bad.
...
HSTDP - WARNING -  Testing expected verbose warning : Force verbose warning.
...
class CrdsLogger:
    ...
    def prepend_crds_filter(self, filter):
        """Prepend a message `filter` to HSTDP log output independent of logging framework."""
        if filter not in self.filters:
            self.filters = [filter] + self.filters

    def append_crds_filter(self, filter):
        """Append a message `filter` to HSTDP log output independent of logging framework."""
        if filter not in self.filters:
            self.filters = self.filters + [filter]

    def remove_crds_filter(self, filter):
        """Remove `filter` from the sequence of HSTDP message filters."""
        if filter in self.filters:
            self.filters.remove(filter)

THE_LOGGER = CrdsLogger("HSTDP")
...
if bool(os.environ.get("HSTDP_TRAP_AUGMENTED", False)):
